         ...
         <div class="dialog">
           ...
           <div class="dialog__content">
            ...
            <div class="mt-3 d-flex align-center" v-if="test.info.params.considerDifficulty">
                ...
                <span class="ml-1">The test takes into account the complexity of the questions</span>
            </div>

            <div class="mt-3 d-flex flex-column" v-if="test.info.params.ballSystem">
                <div class="d-flex align-center">
                    <v-icon color="var(--main-color)" size="19">mdi-check-circle</v-icon>
                    <span class="ml-1">The test takes into account the point system</span>
                </div>
                <div class="d-flex flex-row justify-space-between flex-wrap mt-1">
                    <div><v-icon size="19" color="var(--main-color)">mdi-minus</v-icon>Minimum score: <b>{{ test.info.params.ballSystem.min }}</b></div> 
                    <div><v-icon size="19" color="var(--main-color)">mdi-plus</v-icon>Maximum score: <b>{{ test.info.params.ballSystem.max }}</b></div>
                </div>
            </div>
            ...
            <div class="mt-3 d-flex flex-column">
                <div class="d-flex align-center">
                    ...
                    <span class="ml-1">Total number of questions: <span>{{ test.info.testInfo.totalQuestions }}</span></span>
                </div>
                ...
                <div class="chart-box">
                    <div class="d-flex flex-column" style="gap:5px">
                        <div class="d-flex align-center">
                            ...
                            <span class="ml-1">basic questions: <span style="color: var(--main-color)">{{ test.info.testInfo.bq }}</span></span>
                        </div>
                        <div class="d-flex align-center">
                            <v-icon size="19" color="var(--main-color)">mdi-image-outline</v-icon>
                            <span class="ml-1">questions with images: <span style="color: var(--main-color)">{{ test.info.testInfo.qwi }}</span></span>
                        </div>
                        <div class="d-flex align-center">
                            <v-icon size="19" color="var(--main-color)">mdi-selection-ellipse-arrow-inside</v-icon>
                            <span class="ml-1">questions with field: <span style="color: var(--main-color)">{{ test.info.testInfo.qwf }}</span></span>
                        </div>
                    </div>
...
import { getSubject, getAuthor, getDate, getLanguages, getThemes } from '@/plugins/getInfo'
