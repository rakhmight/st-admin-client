<template>
    <div class="content">
        <title-component :title="'Incoming requests'" />
        ...
        <data-empty :text="`You don't have incoming tests`" v-if="!tests.length" />
    </div>
</template>
    ...
    methods: {
        reRenderTests(){            
            ...
            this.getTestImages.forEach((test, i) => {
                ...
                if(this.getRole==2){
                    if(test.status.value=='under-review' && test.status.step==1) this.tests.push(testI)
                } else if(this.getRole==3) {
                    if(test.status.value=='under-review' && test.status.step==2) this.tests.push(testI)
                }
            })
        },        
